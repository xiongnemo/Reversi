...
int gameover_flag=0;//1代表无合法落子，2代表用户违规操作
int win_flag=0;
int legal_move_flag=1;//0代表不合法

//函数声明开始
void Init_board(void);//读入棋盘大小 n
void printBoard(void);//输出棋盘
void computer_move(int color);//计算机下棋。
void human_move(int color);//用户下棋
int check_board(void);//检测游戏是否结束,1为正常
int check_legal_move(int row,int col,int color);//检测颜色为 color 的棋子落在棋盘格(row, col)上是否合法
int flip(int row,int col,int color);//翻转 8 个方向上的对手棋子
int position_score(int row,int col,int color);//计算“分值”
void gameover(void);//游戏结束，统计得分，输出结果
//函数声明结束
...
{
    if(board[row][col] != 0)//判断此处是否已有棋子
        return 0;
    int i, j, ii, jj;
    int book = 0;
    if(board[row][col + 1] == 3 - color && col <= 5)                  //向右试探
    {
        for(i = col + 2; i < n; i++)
        {
            if(board[row][i] == 0)
    ...
    if(board[row][col - 1] == 3 - color && col >= 2)                  //向左试探
    {
        for(i = col - 2; i >= 0; i--)
        {
            if(board[row][i] == 0)
    ...
    if(board[row - 1][col] == 3 - color && row >= 2)                  //向上试探
    {
        for(i = row - 2; i >= 0; i--)
        {
            if(board[i][col] == 0)
    ...
    if(board[row + 1][col] == 3 - color && row <= 5)                  //向下试探
    {
        for(i = row + 2; i < n; i++)
        {
            if(board[i][col] == 0)
    ...
    if(board[row + 1][col - 1] == 3 - color && (row <= 5 && col >= 2))                  //向左下试探
    {
    ...
    if(board[row - 1][col - 1] == 3 - color && (row >= 2 && col >= 2))                  //向左上试探
    {
    ...
    if(board[row + 1][col + 1] == 3 - color && (row <= 5 && col <= 5))                  //向右下试探
    {
    ...
    if(board[row - 1][col + 1] == 3 - color && (row >= 2 && col <= 5))                  //向右上试探
    {
